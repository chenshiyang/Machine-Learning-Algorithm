...
class BinarySVM:
    ...
    def _SMO(self, K, y):
        ...
        while True:
            ...
            unbounded = [i for i in range(m) if self.eps < self.alphas[i] < self.C - self.eps]
            bounded = [i for i in range(m) if self.alphas[i] <= self.eps or self.alphas[i] >= self.C - self.eps]
            # for i in xrange(m):
    ...
    def _SMO2(self, K, y):
        ...
        def examine_example(i):
            ...
            if (yi * Ei < -tol and ai_old < self.C) or (yi * Ei > tol and ai_old > 0):
                ...
                mid_alpha_ids = filter(lambda j: 0.0 < self.alphas[j] < self.C, range(m))
    ...
    def _SMO5(self, K, y):
        ...
        while True:
            ...
            if aj_new > H:
                ...
                aj_new = L
            delta_j = aj_new - aj_old # 如果谠论迭代 aj没有足够的优化
            if abs(delta_j) < 1e-5:
                # print "j = %d, is not moving enough" % j
                continue
            ai_new = ai_old + yi * yj * (aj_old - aj_new)
            ...
            delta_i = ai_new - ai_old
            self.alphas[i], self.alphas[j] = ai_new, aj_new
            bi = self.b - Ei - yi * delta_i * K[i, i] - yj * delta_j * K[i, j] #TODO 后面的 K[i,j] => K[j,i]
            bj = self.b - Ej - yi * delta_i * K[i, j] - yj * delta_j * K[j, j]
            # 选择满足 0 < a_new < C的b
            # 如果两个都满足, 则b取这两个的均值
            if 0 < ai_new < self.C:
    ...
    def fit(self, x, y):
        ...
        # print type(x), type(y)
        print("Data's shape:", x.shape, y.shape)
        ...
        sv_indices = filter(lambda i: self.alphas[i] > self.eps, range(len(y)))
